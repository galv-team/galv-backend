...
class UserFilterBackend(DRYPermissionFiltersBase):
    ...
    def filter_list_queryset(self, request, queryset, view):
        if (
            ...
            or get_user_auth_details(request).is_lab_admin
        ):
            return queryset
        ...
        for user in all_users:
            if user == request.user or any(
                [
                    lab_id in get_user_auth_details(request).lab_ids
                    for lab_id in UserFilterBackend.user_labs(user)
                ]
            ):
...
class ResourceFilterBackend(DRYPermissionFiltersBase):
    ...
    def filter_list_queryset(self, request, queryset, view):
        ...
        return queryset.filter(
            ...
            | (
                Q(read_access_level=UserLevel.LAB_MEMBER.value)
                & Q(team__lab__pk__in=auth_details.lab_ids)
            )
            |
            # Bit of a hack to chain True/False with Q object
            (
                Q(read_access_level=UserLevel.REGISTERED_USER.value)
                & Q(pk__isnull=not auth_details.is_approved)
            )
...
class ObservedFileFilterBackend(ResourceFilterBackend):
    ...
    def filter_list_queryset(self, request, queryset, view):
        rf_results = (
            super()
            .filter_list_queryset(request, queryset, view)
