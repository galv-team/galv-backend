        ...
        extra_kwargs = augment_extra_kwargs(
            {
                "password": {
                    "write_only": True,
                    "help_text": "Password (8 characters minimum)",
                },
            }
        )
...
@extend_schema_serializer(
    examples=[
        OpenApiExample(
            ...
            value=["http://localhost:8001/users/1/"],
            response_only=True,  # signal that example only applies to responses
        ),
    ]
)
class TransparentGroupSerializer(
            ...
            if hasattr(instance, "editable_lab"):
                if len(validated_data["user_set"]) < 1:
                    raise ValidationError(
                        "Labs must always have at least one administrator"
                    )
        ...
        OpenApiExample(
            ...
            value={
                ...
                "admin_group": {
                    ...
                    "users": ["http://localhost:8001/users/1/"],
                    "permissions": {
                        "create": False,
                        "destroy": False,
                        "write": True,
...
class TeamSerializer(serializers.HyperlinkedModelSerializer, PermissionsMixin):
    member_group = TransparentGroupSerializer(
        required=False, help_text="Members of this Team"
    )
    admin_group = TransparentGroupSerializer(
        required=False, help_text="Administrators of this Team"
    )
    ...
    def validate_lab(self, value):
        ...
        try:
            assert (
                value.pk in self.context["request"].user_auth_details.writeable_lab_ids
            )
    ...
    def create(self, validated_data):
        ...
        if len(admin_group) == 0:
            try:
                ...
                raise ValidationError(
                    "No admins specified and no request context available to determine user."
                )
...
class GalvStorageTypeSerializer(
    serializers.HyperlinkedModelSerializer, PermissionsMixin
):
...
class AdditionalS3StorageTypeSerializer(
    serializers.HyperlinkedModelSerializer, PermissionsMixin, CreateOnlyMixin
):
    bytes_used = serializers.SerializerMethodField()
    secret_key = PasswordField(help_text="Secret key for S3 storage")
    access_key = PasswordField(
        show_first_chars=4, help_text="Access key for S3 storage"
    )
    ...
    def validate_access_key(self, value):
        if self.instance is not None and value in [
            None,
            self.fields["access_key"].to_representation(self.instance.access_key),
        ]:
    ...
    def validate_secret_key(self, value):
        if self.instance is not None and value in [
            None,
            self.fields["secret_key"].to_representation(self.instance.secret_key),
        ]:
...
class LabSerializer(serializers.HyperlinkedModelSerializer, PermissionsMixin):
    admin_group = TransparentGroupSerializer(
        help_text="Group of users who can edit this Lab"
    )
    ...
    def get_storages(self, instance) -> list[str]:
        ...
        return [
            get_storage_url(
                s._meta.model, "detail", args=[s.pk], request=self.context["request"]
            )
            for s in storages
        ]
    ...
    def create(self, validated_data):
        ...
        GalvStorageType.objects.create(
            lab=lab, quota_bytes=settings.LAB_STORAGE_QUOTA_BYTES
        )
        lab.save()
        return lab
    ...
    def validate_team(self, value):
        ...
        try:
            teams = Team.objects.filter(
                pk__in=self.context["request"].user_auth_details.team_ids
            )
            if value is None:
                if len(teams) == 1:
                    value = teams[0]
                    ...
                    raise ValidationError(
                        "You must specify a team because you are a member of multiple teams"
                    )
            else:
                assert value in teams
        except KeyError:
            raise ValidationError(
                "No request context available to determine user's teams"
            )
        except BaseException:
            raise ValidationError(
                "You may only create resources in your own team(s)",
        ...
        if self.instance is not None:
            ...
            if "read_access_level" in attrs or "edit_access_level" in attrs:
                if user_access_level < UserLevel.TEAM_MEMBER.value:
                    raise ValidationError(
                        "You may only change access levels if you are a team member"
                    )
                for access_level in ["read_access_level", "edit_access_level"]:
                    if access_level in attrs:
                        if getattr(self.instance, access_level) > user_access_level:
                            raise ValidationError(
                                f"You may not change {access_level} because your access level is too low"
                            )
            if "delete_access_level" in attrs:
                if user_access_level < UserLevel.TEAM_ADMIN.value:
                    raise ValidationError(
                        "You may only change delete access levels if you are a team admin"
                    )
        if "read_access_level" in attrs:
            edit_level = attrs.get(
                "edit_access_level",
                self.instance.edit_access_level
                if self.instance
                else UserLevel.TEAM_ADMIN.value,
            )
            if attrs["read_access_level"] > edit_level:
                raise ValidationError(
                    "Read access level must be less than or equal to edit access level"
                )
        if "edit_access_level" in attrs:
            delete_level = attrs.get(
                "delete_access_level",
                self.instance.delete_access_level
                if self.instance
                else UserLevel.TEAM_ADMIN.value,
            )
            if attrs["edit_access_level"] > delete_level:
                raise ValidationError(
                    "Edit access level must be less than or equal to delete access level"
                )
        return attrs
...
class CellFamilySerializer(
    CustomPropertiesModelSerializer, PermissionsMixin, WithTeamMixin
):
    manufacturer = GetOrCreateTextField(
        foreign_model=CellManufacturers, help_text="Manufacturer name"
    )
    model = GetOrCreateTextField(foreign_model=CellModels, help_text="Model number")
    chemistry = GetOrCreateTextField(
        foreign_model=CellChemistries, help_text="Chemistry type"
    )
    form_factor = GetOrCreateTextField(
        foreign_model=CellFormFactors, help_text="Physical form factor"
    )
...
class EquipmentFamilySerializer(
    CustomPropertiesModelSerializer, PermissionsMixin, WithTeamMixin
):
    type = GetOrCreateTextField(
        foreign_model=EquipmentTypes, help_text="Equipment type"
    )
    manufacturer = GetOrCreateTextField(
        foreign_model=EquipmentManufacturers, help_text="Manufacturer name"
    )
    model = GetOrCreateTextField(
        foreign_model=EquipmentModels, help_text="Model number"
    )
...
class ScheduleFamilySerializer(
    CustomPropertiesModelSerializer, PermissionsMixin, WithTeamMixin
):
...
class ScheduleSerializer(
    ...
    def validate_pybamm_schedule_variables(self, value):
        ...
        if template is None and value is not None:
            raise ValidationError(
                "pybamm_schedule_variables has no effect if pybamm_template is not set"
            )
        if value is None:
            return value
    ...
    def validate(self, data):
        if data.get("schedule_file") is None:
            try:
                ...
                raise ValidationError(
                    "Schedule_file must be provided where pybamm_template is not set"
                )
        return data
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "sleep_time": 60,
                "environment_variables": {"EXAMPLE_ENV_VAR": "example value"},
                "active": True,
    ...
    class EnvField(serializers.DictField):
        ...
        def to_internal_value(self, values):
            ...
            for k, v in values.items():
                k = k.upper()
                try:
                    env = HarvesterEnvVar.objects.get(
                        harvester=self.root.instance, key=k
                    )
                    env.value = v
                    env.deleted = False
                    env.save()
                except HarvesterEnvVar.DoesNotExist:
                    HarvesterEnvVar.objects.create(
                        harvester=self.root.instance, key=k, value=v
                    )
            envvars = HarvesterEnvVar.objects.filter(
                harvester=self.root.instance, deleted=False
            )
            ...
            for v in envvars.all():
                if v.key not in input_keys:
                    v.deleted = True
                    v.save()
            return HarvesterEnvVar.objects.filter(
                harvester=self.root.instance, deleted=False
            )

    environment_variables = EnvField(
        help_text="Environment variables set on this Harvester"
    )
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "files": [
                    "http://localhost:8001/files/c690ddf0-9527-11ee-8454-eb9d381d3cc4/"
                ],
...
class MonitoredPathSerializer(
    ...
    def get_files(self, instance) -> list[OpenApiTypes.URI]:
        ...
        files = ObservedFile.objects.filter(harvester=instance.harvester).values(
            "path", "id"
        )
        file_urls = []
        for file in files:
    ...
    def validate_harvester(self, value):
        ...
        if value.lab.pk not in get_user_auth_details(request).lab_ids:
            raise ValidationError(
                "You may only create MonitoredPaths on Harvesters in your own lab(s)"
            )
        return value
    ...
    def validate_team(self, value):
        ...
        if value.pk not in self.context["request"].user_auth_details.writeable_team_ids:
            raise ValidationError(
                "You may only create MonitoredPaths in your own team(s)",
                code=HTTP_403_FORBIDDEN,
            )
        return value
    ...
    def validate_stable_time_s(self, value):
        try:
            ...
            raise ValidationError(
                f"stable_time_s value '{value}' is not a positive integer"
            )
    ...
    class Meta:
        ...
        extra_kwargs = augment_extra_kwargs(
            {"harvester": {"create_only": True}, "team": {"create_only": True}}
        )


class ColumnMappingSerializer(
    ...
    def validate_map(self, value):
        ...
        required_column_ids = [
            c.pk for c in DataColumnType.objects.filter(is_required=True)
        ]
        ...
        for k, v in value.items():
            ...
            if not isinstance(k, str):
                raise ValidationError(
                    "Keys must be strings representing the names of columns in the file"
                )
            ...
            if "new_name" in v:
                if column_type.is_required:
                    raise ValidationError(
                        f"Column '{k}' is one of the core required columns and cannot be renamed"
                    )
            ...
            if column_type.data_type in ["int", "float"]:
                ...
                raise ValidationError(
                    f"Column '{k}' is not numerical, so it cannot have a multiplier"
                )
            elif "addition" in v:
                raise ValidationError(
                    f"Column '{k}' is not numerical, so it cannot have an addition"
                )
        return new_value
    ...
    def validate(self, attrs):
        if (
            self.instance
            and self.instance.map != attrs["map"]
            and self.instance.in_use
            and not all(
                [
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "last_sample_no": None,
                "extra_metadata": {
                    "column_0": {"has_data": True},
                    "column_1": {"has_data": True},
                    "column_2": {"has_data": True},
                    "column_3": {"has_data": True},
                    "column_4": {"has_data": True},
                },
...
class ObservedFileCreateSerializer(ObservedFileSerializer, WithTeamMixin):
    ...
    def validate_uploader(self, value):
        ...
        if (
            ...
            and self.instance.uploader != user
    ...
    def validate(self, attrs):
        ...
        if "id" in attrs and "mapping" not in attrs:
            if self.instance is None or self.instance.mapping is None:
                raise ValidationError(
                    "You must specify the mapping when updating an existing file"
                )
        return super().validate(attrs)
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if mapping is not None:
                if mapping not in [
                    m["mapping"]
                    for m in observed_file.applicable_mappings(self.context["request"])
                ]:
...
class DataColumnTypeSerializer(
    serializers.HyperlinkedModelSerializer, WithTeamMixin, PermissionsMixin
):
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "authors": ["http://localhost:8001/userproxies/1/"],
                "protocol": {"detail": "JSON representation of experiment protocol"},
                "protocol_file": None,
                "cycler_tests": [
                    "http://localhost:8001/cycler_tests/2b7313c9-94c2-4276-a4ee-e9d58d8a641b/"
                ],
...
class ExperimentSerializer(
    serializers.HyperlinkedModelSerializer, PermissionsMixin, WithTeamMixin
):
    cycler_tests = TruncatedHyperlinkedRelatedIdField(
        "CyclerTestSerializer",
        ["cell", "equipment", "schedule"],
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "schema": {
                    "type": "object",
                    "properties": {"example_property": {"type": "string"}},
...
class ValidationSchemaSerializer(
    serializers.HyperlinkedModelSerializer, PermissionsMixin, WithTeamMixin
):
...
class KnoxTokenSerializer(serializers.HyperlinkedModelSerializer, PermissionsMixin):
    ...
    expiry = serializers.SerializerMethodField(
        help_text="Date and time token expires (blank = never)"
    )
    ...
    def knox_token(self, instance):
        ...
        return AuthToken.objects.get(
            user=instance.user, token_key=instance.knox_token_key
        )
    ...
    def get_url(self, instance) -> str:
        return reverse(
            "tokens-detail", args=(instance.id,), request=self.context["request"]
        )
...
class HarvesterConfigSerializer(HarvesterSerializer, PermissionsMixin):
    max_upload_bytes = serializers.SerializerMethodField(
        help_text="Maximum upload size (bytes)"
    )
    deleted_environment_variables = serializers.SerializerMethodField(
        help_text="Envvars to unset"
    )
    monitored_paths = MonitoredPathSerializer(
        many=True, read_only=True, help_text="Directories to harvest"
    )
...
class HarvesterCreateSerializer(HarvesterSerializer, PermissionsMixin):
    ...
    def to_representation(self, instance):
        return HarvesterConfigSerializer(context=self.context).to_representation(
            instance
        )
...
class SchemaValidationSerializer(
    serializers.HyperlinkedModelSerializer, PermissionsMixin
):
    ...
    validation_target = serializers.SerializerMethodField(
        help_text="Target of validation"
    )
...
class ArbitraryFileCreateSerializer(ArbitraryFileSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            try:
                arbitrary_file = ArbitraryFile.objects.create(
                    **validated_data, bytes_required=bytes_required
                )
                if file:
