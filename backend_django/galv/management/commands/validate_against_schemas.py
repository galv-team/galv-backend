...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "statuses", nargs="*", type=str, default=[ValidationStatus.UNCHECKED]
        )
    ...
    def handle(self, *args, **options):
        ...
        if len(to_check) == 0:
            self.stdout.write(
                f"No schema validations found with status {'|'.join(statuses)}."
            )
            return
        self.stdout.write(
            f"Found {len(to_check)} schema validations with status {'|'.join(statuses)}."
        )
        statuses = {
        ...
        self.stdout.write(self.style.SUCCESS("Finished checking schema validations."))
        if statuses[ValidationStatus.ERROR]:
            self.stdout.write(
                self.style.ERROR(f"ERROR: {statuses[ValidationStatus.ERROR]}")
            )
        if statuses[ValidationStatus.VALID]:
            self.stdout.write(
                self.style.SUCCESS(f"VALID: {statuses[ValidationStatus.VALID]}")
            )
        if statuses[ValidationStatus.INVALID]:
            self.stdout.write(
                self.style.WARNING(f"INVALID: {statuses[ValidationStatus.INVALID]}")
            )
