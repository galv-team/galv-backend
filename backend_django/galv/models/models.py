...
def get_user_auth_details(request):
    ...
    if request.user is not None:
        if is_harvester:
            ...
            for values in request.user.harvester.monitored_paths.values(
                "team__pk", "team__lab__pk"
            ):
            ...
            for g in request.user.groups.values(
                ...
                "readable_team__pk",
                "editable_lab__pk",
                "editable_team__lab__pk",
                "readable_team__lab__pk",
            ):
...
class UserActivation(TimestampedModel):
    ...
    token = models.CharField(max_length=token_length, null=True, blank=True)
    token_update_date = models.DateTimeField(null=True, blank=True)
    ...
    def generate_token(self):
        self.token = get_random_string(
            length=self.token_length, allowed_chars="1234567890"
        )
    ...
    def get_is_expired(self) -> bool:
        return (
            self.token_update_date is None
            or (timezone.now() - self.token_update_date).total_seconds()
            > settings.USER_ACTIVATION_TOKEN_EXPIRY_S
        )
    ...
    def activate_user(self):
        if self.get_is_expired():
            self.generate_token()
            raise ValueError(
                "Activation token expired. A new token has been generated and emailed to you."
            )
...
class UserProxy(User):
    ...
    def has_object_read_permission(self, request):
        ...
        for g in GroupProxy.objects.filter(user=self):
            ...
            if (
                isinstance(t, Team)
                and t.lab.pk in get_user_auth_details(request).lab_ids
            ):
                return True
        return False
    ...
    @property
    def activation_requires_approval(self) -> bool:
...
class GroupProxy(Group):
    ...
    def has_object_write_permission(self, request):
        ...
        if owner is not None:
            return owner.has_object_write_permission(
                request
            )  # or self in request.user.groups.all()
        return False
    ...
    def has_object_read_permission(self, request):
        ...
        if owner is not None:
            return (
                owner.has_object_read_permission(request)
                or self in request.user.groups.all()
            )
...
class _StorageTypeConsumerModel(UUIDModel):
    ...
    _storage_content_type = models.ForeignKey(
    ...
    special_dump_fields = {"_storage_content_type": None, "_storage_object_id": None}
...
class _StorageType(UUIDModel):
    ...
    lab = models.ForeignKey(
        "Lab", related_name="storage_%(class)s", on_delete=models.CASCADE
    )
        ...
        if adding:
            ...
            if (
                ...
                raise StorageFullError(
                    f"Cannot save data: local storage quota exceeded for {self}"
                )
        try:
            ...
            raise StorageConfigurationError(
                f"Could not configure storage for {self}"
            ) from e
...
class AdditionalS3StorageType(_StorageType):
    ...
    access_key = models.TextField(
        null=True, blank=True, help_text="Access key for the S3 bucket"
    )
    secret_key = models.TextField(
        null=True, blank=True, help_text="Secret key for the S3 bucket"
    )
    ...
    custom_domain = models.TextField(
        null=True, blank=True, help_text=("Custom domain for the S3 bucket.")
    )
    ...
    def get_storage(self, instance, adding=False) -> Storage:
        if adding:
            ...
            if (
                ...
                raise StorageFullError(
                    f"Cannot save data: storage quota exceeded for {self}"
                )
        try:
            return S3DataStorage(
...
class Lab(TimestampedModel):
    name = models.TextField(unique=True, help_text="Human-friendly Lab identifier")
    description = models.TextField(null=True, help_text="Description of the Lab")
    ...
    def has_object_read_permission(self, request):
        return (
            ...
            or self.pk in get_user_auth_details(request).lab_ids
        )
    ...
    def has_object_write_permission(self, request):
        return (
            request.user.is_staff
            or request.user.is_superuser
            or self.pk in get_user_auth_details(request).writeable_lab_ids
        )
...
class Team(TimestampedModel):
    name = models.TextField(unique=False, help_text="Human-friendly Team identifier")
    description = models.TextField(null=True, help_text="Description of the Team")
    lab = models.ForeignKey(
        to=Lab,
        on_delete=models.CASCADE,
        null=False,
    ...
    def has_object_read_permission(self, request):
        return (
            self.lab.pk in get_user_auth_details(request).writeable_lab_ids
            or self.pk in get_user_auth_details(request).team_ids
        )

    def has_object_write_permission(self, request):
        return (
            self.lab.pk in get_user_auth_details(request).writeable_lab_ids
            or self.pk in get_user_auth_details(request).writeable_team_ids
        )
    ...
    def save(
        ...
        if self.admin_group is None or self.member_group is None:
            if self.admin_group is None:
                ...
                self.admin_group = GroupProxy.objects.create(
                    name=f"Team {self.pk} admins"
                )
            if self.member_group is None:
                self.member_group = GroupProxy.objects.create(
                    name=f"Team {self.pk} members"
                )
            self.save()
    ...
    def register_validation(self):
        for schema in ValidationSchema.objects.all():
            SchemaValidation.objects.update_or_create(
                defaults={"status": ValidationStatus.UNCHECKED, "detail": None},
                schema=schema,
...
class BibliographicInfo(TimestampedModel):
    ...
    bib_json = models.JSONField(null=False, blank=False, default=dict)
    ...
    def has_create_permission(request):
        return (
            get_user_auth_details(request).is_authenticated
            and get_user_auth_details(request).is_approved
        )
...
class CellFamily(CustomPropertiesModel, ResourceModelPermissionsMixin):
    ...
    datasheet = models.URLField(
        help_text="Link to the datasheet", null=True, blank=True
    )
    nominal_voltage_v = models.FloatField(
        help_text="Nominal voltage of the cells (in volts)", null=True, blank=True
    )
    nominal_capacity_ah = models.FloatField(
        help_text="Nominal capacity of the cells (in amp hours)", null=True, blank=True
    )
    initial_ac_impedance_o = models.FloatField(
        help_text="Initial AC impedance of the cells (in ohms)", null=True, blank=True
    )
    initial_dc_resistance_o = models.FloatField(
        help_text="Initial DC resistance of the cells (in ohms)", null=True, blank=True
    )
    energy_density_wh_per_kg = models.FloatField(
        help_text="Energy density of the cells (in watt hours per kilogram)",
        null=True,
        blank=True,
    )
    power_density_w_per_kg = models.FloatField(
...
class Equipment(JSONModel, ResourceModelPermissionsMixin, ValidatableBySchemaMixin):
    ...
    calibration_date = models.DateField(
        help_text="Date of last calibration", null=True, blank=True
    )
    ...
    def __json_ld__(self):
        return {
            ...
            f"{LDSources.SCHEMA}:manufacturer": str(
                self.family.manufacturer.__json_ld__()
            ),
        }
...
class Schedule(JSONModel, ResourceModelPermissionsMixin, ValidatableBySchemaMixin):
    ...
    schedule_file = models.FileField(
        help_text="File containing the schedule", null=True, blank=True
    )
    pybamm_schedule_variables = models.JSONField(
...
class Harvester(UUIDModel):
    ...
    api_key = models.TextField(
        null=True, help_text="API access token for the Harvester"
    )
    last_check_in = models.DateTimeField(
        null=True, help_text="Date and time of last Harvester contact"
    )
    last_check_in_job = models.TextField(
        null=True, help_text="Job description of last Harvester contact"
    )
    sleep_time = models.IntegerField(
        default=120, help_text="Seconds to sleep between Harvester cycles"
    ...
    def has_create_permission(request):
        return (
            get_user_auth_details(request).is_authenticated
            and len(get_user_auth_details(request).writeable_lab_ids) > 0
        )
    ...
    def has_object_read_permission(self, request):
        return self.is_valid_harvester(request) or self.lab.has_object_read_permission(
            request
        )
    ...
    def is_valid_harvester(self, request):
        return (
            isinstance(request.user, HarvesterUser) and request.user.harvester == self
        )
...
class ObservedFile(
    _StorageTypeConsumerModel, ValidatableBySchemaMixin, ResourceModelPermissionsMixin
):
    ...
    last_observed_time = models.DateTimeField(
        null=True, help_text="Date and time of last Harvester report on file"
    )
    ...
    inferred_format = models.TextField(null=True, help_text="Format of the raw data")
    name = models.TextField(null=True, help_text="Name of the file")
    parser = models.TextField(null=True, help_text="Parser used by the harvester")
    ...
    last_sample_no = models.PositiveIntegerField(
        null=True, help_text="Number of the last sample in the file"
    )
    core_metadata = models.JSONField(
        null=True, help_text="Unparsed core metadata from the harvester"
    )
    extra_metadata = models.JSONField(
        null=True, help_text="Extra metadata from the harvester"
    )
    ...
    def has_create_permission(request):
        try:
            ...
            return Team.objects.filter(
                pk__in=get_user_auth_details(request).team_ids
            ).exists()
    ...
    def has_object_read_permission(self, request):
        ...
        return any(
            [
                path.has_object_read_permission(request)
                for path in self.monitored_paths.all()
            ]
        )
    ...
    def has_object_write_permission(self, request):
        ...
        return any(
            [
                path.has_object_write_permission(request)
                for path in self.monitored_paths.all()
            ]
        )
    ...
    def applicable_mappings(self, request):
        ...
        for mapping in ColumnMapping.objects.all():
            ...
            if mapping.is_valid:
                applicable_valid_mappings.append(
                    {"mapping": mapping, "missing": missing}
                )
            else:
                applicable_invalid_mappings.append(
                    {"mapping": mapping, "missing": missing}
                )

        applicable_valid_mappings = sorted(
            applicable_valid_mappings, key=lambda x: x["missing"]
        )
        applicable_invalid_mappings = sorted(
            applicable_invalid_mappings, key=lambda x: x["missing"]
        )
...
class CyclerTest(JSONModel, ResourceModelPermissionsMixin, ValidatableBySchemaMixin):
    ...
    files = models.ManyToManyField(
        to=ObservedFile, help_text="Test data", related_name="cycler_tests"
    )
...
class Experiment(JSONModel, ResourceModelPermissionsMixin, ValidatableBySchemaMixin):
    ...
    description = models.TextField(
        help_text="Description of the experiment", null=True, blank=True
    )
    authors = models.ManyToManyField(
        to=UserProxy, help_text="Authors of the experiment"
    )
    protocol = models.JSONField(
        help_text="Protocol of the experiment", null=True, blank=True
    )
    protocol_file = models.FileField(
        help_text="Protocol file of the experiment", null=True, blank=True
    )
    cycler_tests = models.ManyToManyField(
        to=CyclerTest,
...
class HarvesterEnvVar(TimestampedModel):
    ...
    deleted = models.BooleanField(
        help_text="Whether this variable was deleted", default=False, null=False
    )
...
class DataUnit(ResourceModelPermissionsMixin):
    name = models.TextField(null=False, help_text="Common name")
    symbol = models.TextField(null=False, help_text="Symbol")
    description = models.TextField(
...
class DataColumnType(ResourceModelPermissionsMixin, ValidatableBySchemaMixin):
    ...
    is_required = models.BooleanField(
        default=False, help_text="Whether the Column must be present in every Dataset"
    )
...
class TimeseriesRangeLabel(TimestampedModel):
    ...
    label = models.TextField(null=False, help_text="Human-friendly identifier")
    range_start = models.PositiveBigIntegerField(
        null=False, help_text="Row (sample number) at which the range starts"
    )
    range_end = models.PositiveBigIntegerField(
        null=False, help_text="Row (sample number) at which the range ends"
    )
...
class SchemaValidation(TimestampedModel):
    ...
    status = models.TextField(
        null=False, help_text="Validation status", choices=ValidationStatus.choices
    )
    detail = models.JSONField(null=True, help_text="Validation detail")
    last_update = models.DateTimeField(
        auto_now=True, null=False, help_text="Date and time of last status update"
    )
    ...
    def __str__(self):
        return (
            f"{self.validation_target.__str__} vs {self.schema.__str__}: {self.status}"
        )
    ...
    def validate(self, halt_on_error=False):
        ...
        try:
            ...
            data = serializer(
                self.validation_target, context={"request": mock_request}
            ).data
...
class ArbitraryFile(_StorageTypeConsumerModel, ResourceModelPermissionsMixin):
    file = LabDependentStorageFileField(null=True, blank=True, help_text="File")
    name = models.TextField(help_text="The name of the file", null=False, blank=False)
    description = models.TextField(
        help_text="The description of the file", null=True, blank=True
    )
