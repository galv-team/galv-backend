        ...
        * Create requests allowed for lab admins only, and only if the lab is their lab
        ...
        for user, details in {
            ...
            with self.subTest(user=user):
                ...
                response = self.client.post(url, {**create_dict, 'lab': reverse('lab-detail', args=(self.lab.pk,))})
                self.assertTrue(
    ...
    def test_list(self):
        ...
        for user, details in {
            ...
            'strange_lab_admin': {
                ...
                'response': lambda r: len(r.json()['results']) == int(self.__class__.__name__ == 'GalvStorageTypeTests')
            },
            'anonymous': {
                'login': lambda: self.client.logout(),
