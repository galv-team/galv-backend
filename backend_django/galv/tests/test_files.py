...
class ObservedFileTests(GalvTestCase):
    ...
    def setUp(self):
        ...
        self.regex_files = ObservedFileFactory.create_batch(
            size=6, harvester=self.harvester, path_root=f"{self.regex_path.path}/abc"
        )
        self.other_harvester = HarvesterFactory.create(
            name="Test Files Other", lab=self.strange_lab
        )
        self.other_harvester_path = MonitoredPathFactory.create(
            harvester=self.other_harvester, path="/", team=self.strange_lab_team
        )
        self.other_harvester_files = ObservedFileFactory.create_batch(
            size=4,
            harvester=self.other_harvester,
            path_root=self.other_harvester_path.path,
        )
    ...
    def test_list(self):
        for user, details in {
            "user": {
                ...
                "expected_set": [*self.specific_files, *self.regex_files],
            },
            "admin": {
            ...
            "stranger": {
                ...
                "expected_set": [*self.other_harvester_files],
            },
            ...
            with self.subTest(user=user):
                ...
                self.assertEqual(
                    ...
                    len(details["expected_set"]),
                )
                for file in details["expected_set"]:
                    self.assertIn(
                        str(file.id),
                        [p["id"] for p in response.json().get("results", [])],
                    )
        ...
        for user, details in {
            ...
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse(f"{self.stub}-detail", args=(self.specific_files[0].id,))
                )
                assert_response_property(
                    self,
                    response,
                ...
                if response.status_code == 200:
                    self.assertEqual(
                        response.json()["id"], str(self.specific_files[0].id)
                    )
        ...
        for user, login in {
            ...
            with self.subTest(user=user):
                ...
                response = self.client.delete(
                    reverse(f"{self.stub}-list"), data=self.get_edit_kwargs()
                )
                assert_response_property(
                    self,
                    response,
    ...
    def test_reimport(self):
        for user, details in {
            ...
            with self.subTest(user=user):
                ...
                details["login"]()
                response = self.client.get(
                    reverse(f"{self.stub}-reimport", args=(self.specific_files[0].id,))
                )
                assert_response_property(
                    self,
                    response,
    ...
    def test_create(self):
        ...
        for user, details in {
            ...
            with tempfile.TemporaryFile() as f:
                ...
                with self.subTest(
                    ...
                    details["login"]()
                    data = {
                        **get_upload_data(user if isinstance(user, str) else user.id),
                        "file": f,
                    }
                    response = self.client.post(
                        reverse(f"{self.stub}-list"), data=data, format="multipart"
                    )
                    assert_response_property(
                        self,
                        response,
        ...
        with self.subTest("JSON not allowed"):
            with tempfile.TemporaryFile() as f:
                ...
                response = self.client.post(
                    reverse(f"{self.stub}-list"), data=data, format="json"
                )
                assert_response_property(
                    self, response, self.assertEqual, response.status_code, 415
                )
        ...
        with self.subTest("Two-stage upload"):
            ...
            with self.subTest("Stage one - no mapping"):
                with tempfile.TemporaryFile() as f:
                    ...
                    assert_response_property(
                        self, response, self.assertEqual, response.status_code, 201
                    )
            ...
            with self.subTest("Stage two - with mapping"):
                ...
                with tempfile.TemporaryFile() as f:
                    ...
                    response = self.client.post(
                        ...
                        {
                            **get_upload_data(self.user.id),
                            "target_file_id": str(observed_file.id),
                            "file": f,
                        },  # includes the mapping this time
                        format="multipart",
                    )
                    assert_response_property(
                        self, response, self.assertEqual, response.status_code, 201
