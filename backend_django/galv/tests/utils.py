...
class GalvTestCase(APITestCaseWrapper):
    ...
    def __init__(self, *args, **kwargs):
        abstract = self.__class__.__name__ == "GalvTestCase" or kwargs.pop(
            "abstract", False
        )
        ...
        if abstract:
            return
        if (
            self.edit_kwargs is None
            and type(self).get_edit_kwargs == GalvTeamResourceTestCase.get_edit_kwargs
        ):
        ...
        if self.stub is None:
            raise AssertionError(
                f"Children of {self.__class__.__name__} must define a self.stub"
            )
        if self.factory is None:
            raise AssertionError(
                f"Children of {self.__class__.__name__} must define a self.factory"
            )
    ...
    def create_resource_users(self) -> None:
        ...
        self.lab_other_team = TeamFactory.create(
            name=f"{prefix} Other Team", lab=self.lab
        )
        self.strange_lab = LabFactory.create(name=f"{prefix} Strange Lab")
        self.strange_lab_team = TeamFactory.create(
            name=f"{prefix} Strange Lab Team", lab=self.strange_lab
        )
        self.strange_lab_admin = UserFactory.create(
            username=f"{prefix}_strange_lab_admin"
        )
    ...
    def assertResourceInResults(
        self, resource, result, assert_single_result=True, assert_reachable=True
    ):
        ...
        if assert_single_result and len(matched_result) != 1:
            raise AssertionError(
                f"Expected single instance of {resource}, got {len(matched_result)} instances"
            )
        ...
        if result.status_code < 200 or result.status_code >= 400:
            raise AssertionError(
                f"Could not reach {url} for {resource} (HTTP {result.status_code})"
            )
...
class _GalvTeamResourceTestCase(GalvTestCase):
    ...
    def create_test_resources(self):
        ...
        self.access_test_lab_no_read = self.create_with_perms(
            read_access_level=UserLevel.TEAM_MEMBER.value
        )
        self.access_test_lab_write = self.create_with_perms(
            edit_access_level=UserLevel.LAB_MEMBER.value
        )
        self.access_test_authorised_read = self.create_with_perms(
        ...
        self.access_test_open = self.create_with_perms(
    ...
    def file_safe_request(self, request_method, url, content, **kwargs):
        ...
        """
        return_value = None
        if "schedule_file" in content or any(
            [isinstance(v, File) for v in content.values()]
        ):
    ...
    * Delete: TEAM_MEMBER
        ...
        for user, login in {
            ...
            with self.subTest(user=user):
                ...
                assert_response_property(
                    ...
                    400,
                    msg=f"Check can't create resources on {self.lab_team}",
                )
        ...
        for user in [self.admin, self.user]:
            with self.subTest(username=user.username):
                ...
                assert_response_property(
                    ...
                    response.status_code,
                    201,
                    msg=f"Check {user.username} can create resources on {self.lab_team}",
                )
    ...
    def test_read_anonymous(self):
        ...
        * Read requests allowed if any of:
        ...
        assert_response_property(
            self, response, self.assertEqual, response.status_code, 200
        )
        ...
        if isinstance(self.access_test_default.pk, UUID):
            with self.subTest("Test dump"):
                response = self.client.get(
                    reverse("dump", args=(self.access_test_default.pk,))
                )
                assert_response_property(
                    self, response, self.assertEqual, response.status_code, 200
                )
                if (
                    hasattr(self.access_test_default, "special_dump_fields")
                    and self.access_test_default.special_dump_fields
                ):
        ...
        for resource in [
            ...
            with self.subTest(resource=self.get_resource_description(resource)):
                ...
                assert_response_property(
                    ...
                    response.status_code,
                    401,
                    msg=f"Check anonymous can't update resources on {self.lab_team}",
                )
        ...
        for resource, code in [
            ...
            with self.subTest(resource=self.get_resource_description(resource)):
                ...
                assert_response_property(
                    ...
                    response.status_code,
                    code,
                    msg=f"Check {self.strange_lab_admin.username} gets HTTP {code} on {resource} [got {response.status_code} instead]",
                )
        ...
        for resource, code in [
            ...
            with self.subTest(resource=self.get_resource_description(resource)):
                ...
                assert_response_property(
                    ...
                    response.status_code,
                    code,
                    msg=f"Check {self.lab_admin.username} gets HTTP {code} on {resource} [got {response.status_code} instead]",
                )
        ...
        for user, login in {
            ...
            for perms, code in [
                (
                    ...
                    403 if user != "anonymous" else 401,
                ),
                (
                    {"delete_access_level": UserLevel.TEAM_ADMIN.value},
                    403 if user != "anonymous" else 401,
                ...
                with self.subTest(user=user, perms=perms):
                    ...
                    assert_response_property(
                        ...
                        response.status_code,
                        code,
                        msg=f"Check can't delete resources on {self.lab_team}",
                    )
        ...
        * Delete requests allowed if:
        ...
        for user in [self.admin, self.user]:
            for perms, code in [
                ...
                (
                    {"delete_access_level": UserLevel.TEAM_ADMIN.value},
                    204 if user == self.admin else 403,
                ),
            ]:
                with self.subTest(username=user.username, perms=perms):
                    ...
                    assert_response_property(
                        ...
                        response.status_code,
                        code,
                        msg=f"Check {user.username} gets {code} deleting resources on {self.lab_team}",
                    )
