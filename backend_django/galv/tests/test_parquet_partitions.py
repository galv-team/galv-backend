...
class ParquetPartitionTests(GalvTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.harvester = HarvesterFactory.create(
            name="Test Parquet Partitions", lab=self.lab
        )
        ...
        self.other_partition = ParquetPartitionFactory.create(
            observed_file=self.other_file
        )
        ...
        for user, details in {
            ...
            with self.subTest(user=user):
                ...
                response = self.client.get(
                    reverse(f"{self.stub}-detail", args=(self.partition.id,))
                )
                assert_response_property(
                    self,
                    response,
